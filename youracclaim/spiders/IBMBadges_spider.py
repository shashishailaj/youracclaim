...
class IBMBadges_spider(scrapy.Spider):  
    ...
    def parse(self, response):
    	...
    	for badge_info in badge_list:
    		...
    		item["keywords"] = self.parse_item(item["link"])
    		yield item
    ...
    def parse_item(self,link):
        ...
        arr = []
        for keyword in soup.find_all('li', 'tag'):
            #print("skill -> " + skill.get_text().strip())
            arr.append(keyword.get_text().strip())
